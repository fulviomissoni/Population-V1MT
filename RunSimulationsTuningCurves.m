...
stim.disp = 0;
[tt,vv] = meshgrid(linspace(0,pi,n_orient),param.pref_vel*2);
stim.truetheta = tt(:);
% stim.truetheta = 0;
stim.vstim =  vv(:); %velocity in px/sec; 
stim.dur = 127;
% stim.theta_g = [0, 0];
...
param.norm_param = [alpha1, alpha2];
x = linspace(1,n_orient,100);
sigma_orient = 10; %means that I will pool over all the orientation in the normalisation stage
x_8 = round(linspace(1,100,n_orient));
